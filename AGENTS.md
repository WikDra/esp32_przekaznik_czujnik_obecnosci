...
| **Historia wykryć (`/api/events`)** | działa: pełny cykl w kolejności od najnowszego (`light_on src=web`, `presence dist=254`, `light_off src=web`, `presence_end duration_s=6`); po rozszerzeniu `capacity=200`, stronicowanie `?limit=`/`?offset=` zwraca rozłączne strony, heap stabilny ~100,8 kB po serii odpytań (odpowiedź strumieniowana porcjami po 16 wpisów) |
